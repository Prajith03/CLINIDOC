...
export const ChartContainer = React.forwardRef<HTMLDivElement, ChartContainerProps>(
  ({ config, className, children, ...props }, ref) => {
    ...
    const style = Object.entries(config).reduce(
      (acc, [key, value]) => {
        const color = (value as any).color || (value as any).theme?.light || (value as any).theme?.dark
        if (color) {
          acc[`--color-${key}`] = color
        }
        return acc
...
export const ChartTooltip = React.forwardRef<HTMLDivElement, React.ComponentProps<typeof Tooltip>>(
    ...
    <div className="rounded-lg border bg-background p-2 shadow-sm">
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="flex flex-col gap-1">
          {payload.map((item) => {
            const color = config[item.dataKey]?.color || `var(--color-${item.dataKey})`
